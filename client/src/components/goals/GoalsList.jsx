...
function GoalsList() {
  const { goals, loading, refreshData } = useFinance();
  const { user } = useAuth();
  ...
  // Remove duplicates based on goal ID (in case there are any)
  const uniqueGoals = goals.filter(
    (goal, index, self) => index === self.findIndex((g) => g._id === goal._id)
  );
  ...
  const completedGoals = uniqueGoals.filter(
    (goal) => goal.isCompleted || false
  );
  const activeGoals = uniqueGoals.filter(
    (goal) => !(goal.isCompleted || false)
  );

  // Get unique currencies from goals for filter dropdown
  const availableCurrencies = [
    ...new Set(uniqueGoals.map((goal) => goal.currency || currency)),
  ];
    ...
    <div className="card animate-slide-up">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-sm text-[var(--light-text)]">
          Total: {uniqueGoals.length} | Completed: {completedGoals.length} |
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <button
          ...
          disabled={isExporting || uniqueGoals.length === 0}
